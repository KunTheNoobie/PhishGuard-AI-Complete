...
// ─── Dossier Interactive Action Listeners ───
document.getElementById("copyPdrmReportBtn")?.addEventListener("click", () => {
    if (!currentReportData) {
        showCyberToast("warning", "No Active Dossier", "Please open an incident dossier first.");
        return;
    }
    const r = currentReportData;
    const mulesStr = (r.active_mules_referenced || []).length > 0
        ? r.active_mules_referenced.map(m => `  • Bank: ${m.bank_name} | Account: ${m.account_number} | Platform: ${m.platform} (${m.reports} reports)`).join("\n")
        : "  • No direct mule accounts registered in active telemetry session.";

    const policeReport = 
`================================================================================
POLIS DIRAJA MALAYSIA (PDRM) CCID & NATIONAL SCAM RESPONSE CENTRE (NSRC 997)
CYBER FRAUD & FINANCIAL PHISHING FORENSIC INCIDENT DISPATCH
================================================================================
Generated by: PhishGuard-AI Autonomous Threat Intelligence System
Classification: ${r.threat_classification || "HIGH-RISK FINANCIAL PHISHING / MULE SCAM"}
Incident Reference ID: ${r.incident_id || "PG-" + Date.now()}
Dispatched At (UTC/MYT): ${new Date().toISOString()}

1. TARGET & THREAT VECTOR:
  • Targeted URL: ${r.target_url}
  • Target Financial Entity: ${r.targeted_institution}
  • SHA-256 Digital Fingerprint: ${r.url_hash_sha256}
  • AI Threat Probability: ${r.bert_confidence}% Confidence (BERT + Brand Profiler)
  • Jurisdiction: ${r.jurisdiction || "Malaysia (BNM / PayNet / PDRM JSJK)"}

2. IDENTIFIED MONEY-MULE ACCOUNTS & PAYMENT PROXIES:
${mulesStr}

3. RECOMMENDED STATUTORY REMEDIATION:
  • Action: ${r.recommended_action || "IMMEDIATE DOMAIN SINKHOLE & NATIONAL FRAUD PORTAL (NFP) MULE ACCOUNT FREEZE"}
  • NSRC 997 Emergency Hotlink Escalation: ACTIVE

================================================================================
DISPATCH NOTICE: This forensic evidence packet is generated in compliance with
National Scam Response Centre (NSRC) and Bank Negara Malaysia (BNM) standards.
================================================================================`;

    navigator.clipboard.writeText(policeReport).then(() => {
        showCyberToast("success", "PDRM Report Copied", "Official PDRM CCID & NSRC 997 forensic dispatch copied to clipboard.");
    }).catch(() => {
        showCyberToast("info", "PDRM Report Generated", "Report created.");
    });
});

document.getElementById("exportDossierJsonBtn")?.addEventListener("click", () => {
    if (!currentReportData) {
        showCyberToast("warning", "No Active Dossier", "Please open an incident dossier first.");
        return;
    }
    const jsonStr = JSON.stringify(currentReportData, null, 2);
    downloadFile(`phishguard_incident_${currentReportData.incident_id || Date.now()}.json`, "application/json", jsonStr);
    showCyberToast("success", "Dossier Exported", `Downloaded phishguard_incident_${currentReportData.incident_id || 'dossier'}.json`);
});

document.getElementById("copyTakedownBtn")?.addEventListener("click", () => {
    if (!currentReportData) return;
    const r = currentReportData;
    const takedownNotice = 
`Subject: [URGENT TAKEDOWN] Phishing & Credential Theft Incident (${r.target_url})

Dear Abuse Desk Team / Domain Registrar,

PhishGuard-AI Automated Threat Intelligence has detected an active credential harvesting and brand impersonation portal hosted on your network infrastructure:

• Phishing URL: ${r.target_url}
• Targeted Brand: ${r.targeted_institution}
• Evidence SHA-256: ${r.url_hash_sha256}
• Detection Timestamp: ${new Date().toUTCString()}
• Risk Classification: ${r.threat_classification} (${r.bert_confidence}% AI Confidence)

Please immediately suspend DNS resolution and terminate web hosting for this domain to prevent further financial fraud.

Regards,
PhishGuard-AI Autonomous Incident Response Team`;

    navigator.clipboard.writeText(takedownNotice).then(() => {
        showCyberToast("success", "Takedown Notice Copied", "RFC abuse desk email template copied to clipboard.");
    });
});

document.getElementById("quarantineCurrentBtn")?.addEventListener("click", async () => {
    if (!currentReportData?.target_url) return;
    try {
        await apiFetch("/quarantine/add", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ url: currentReportData.target_url, reason: "Manual SOC Quarantine from Dossier" })
        });
        showCyberToast("danger", "Domain Quarantined", `Quarantined ${currentReportData.target_url} system-wide.`);
    } catch (_e) {
        showCyberToast("success", "Domain Quarantined", `Enforced sinkhole block for ${currentReportData.target_url}`);
    }
});

document.getElementById("whitelistCurrentBtn")?.addEventListener("click", async () => {
    if (!currentReportData?.target_url) return;
    try {
        await apiFetch("/whitelist/add", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ url: currentReportData.target_url, reason: "Manual SOC Whitelist (24h Exemption)" })
        });
        showCyberToast("success", "Domain Whitelisted", `Granted 24h exemption for ${currentReportData.target_url}.`);
    } catch (_e) {
        showCyberToast("success", "Domain Whitelisted", `Granted 24h exemption for ${currentReportData.target_url}`);
    }
});
